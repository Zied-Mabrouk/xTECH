...
const Dictaphone = ({ active, setMessage, onSend }) => {
  let [command, setCommand] = React.useState("");
  const commands = [
    {
      command: ["write", "right", "bright", "all right"],
      callback: () => setCommand(prev =>{
        if(!prev)
            return "write";
        return prev;
      }),
      isFuzzyMatch: true,
      fuzzyMatchingThreshold: 0.01,
    },
    {
      command: [
        "send",
        ...
        "send the message now please",
      ],
      callback: () => setCommand("send"),
      isFuzzyMatch: true,
  ...
  React.useEffect(() => {
    ...
    setCommand(null);
  }, [command, resetTranscript,onSend]);
  ...
  React.useEffect(() => {
    ...
    resetTranscript();
  }, [listening, active, transcript, setMessage, resetTranscript, command]);
